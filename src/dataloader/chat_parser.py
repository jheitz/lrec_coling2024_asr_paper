...
class ChatTranscriptParser:
    ...
    def _match_relevant_utterances(self, transcript, only_PAR, return_timing=False):
        ...
        # If only_PAR==False, then also utterances with *INV are collected
        # If return_timing==True, the timing information is also returned
        ...
        if only_PAR:
            parts = [p for p in parts if p['speaker'] == 'PAR']

        if return_timing:
            return parts
        else:
            # only return transcript, no timing and speaker information
            return [p['transcript'] for p in parts]

    def preprocess_transcript(self, transcript):
        parts = self._match_relevant_utterances(transcript, only_PAR=self.config['only_PAR'])
        parts = [self._preprocess_utterance(p) for p in parts]

        # only keep non-empty parts
        parts = [s for s in parts if len(s.strip()) > 0]
    ...
    def extract_disfluency_metrics(self, transcript):
        ...
        parts = self._match_relevant_utterances(transcript, only_PAR=False)
        parts_metrics = [self._extract_disfluency_metrics_from_utterance(p) for p in parts]

        # combine
        collected = {}
        metric_names = parts_metrics[0].keys()
        total_disfluency_count = 0
        for m in metric_names:
            sum_over_utterances = sum([p[m] for p in parts_metrics])
    ...
    def extract_speaker_segmentation(self, transcript):
        ...
        """
        parts = self._match_relevant_utterances(transcript, only_PAR=self.config['only_PAR'], return_timing=True)

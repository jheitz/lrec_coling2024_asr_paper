...
class LinguisticFeaturesLiterature(Preprocessor):
    """
    ...
    def _calculate_for_text(self, text):
        ...
        feature_calculator = LinguisticFeatureCalculator(doc, constants=self.CONSTANTS)
    ...
    def _load_features(self, dataset: TextDataset):
        assert isinstance(dataset, TextDataset), "Input should be TextDataset (transcripts)"
        if 'transcript_config' in dataset.config:
            ...
            text_has_terminators = transcript_config['keep_terminators'] or 'Automatic Punctuation' in dataset.config['preprocessors']
            assert text_has_terminators, "If there are no terminators (periods), some of the linguistic metrics don't make sense"
            assert not transcript_config['keep_pauses'], "Don't use linguistic features if there are explicit pauses, this might mess it up"
        ...
        return TabularDataset(data=features_df, labels=np.array(dataset.labels),
